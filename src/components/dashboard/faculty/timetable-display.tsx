    ...
    return (
      <Card className="h-full flex flex-col items-center justify-center text-center">
        <CardHeader>
            ...
            <CardDescription>Your AI-generated schedule will appear here once it's created.</CardDescription>
        </CardHeader>
        <CardContent>
            <p className="text-muted-foreground">Fill out the form on the left and click "Generate Timetable" to begin.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card className="w-full">
      <CardHeader>
        <div className="flex items-center justify-between">
            <div>
                <CardTitle className="font-headline text-2xl">Generated Weekly Timetable</CardTitle>
                <CardDescription>Review the schedule below. If it looks good, publish it to make it live for students.</CardDescription>
            </div>
            ...
            <tbody>
              {times.map((time) => (
                <tr key={time} className="border-b last:border-b-0">
                  <td className="p-3 font-medium text-muted-foreground bg-muted/20">{time}</td>
                  {days.map((day) => {
                    ...
                    return (
                      <td key={`${time}-${day}`} className={`p-3 border-l ${isBreak ? 'bg-amber-100/50 dark:bg-amber-900/30' : ''}`}>
                        {isBreak ? (
